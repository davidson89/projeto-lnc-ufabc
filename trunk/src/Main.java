...
public class Main {
	...
	{		   
	    // Load from 'FCL' file
	    String fileName = "fcl/SistemaBombeio.fcl";
	    ...
	    // Set inputs
	   /* functionBlock.setVariable("EV", 0.5);	// Eficiência Volumétrica [0..1]
	    functionBlock.setVariable("NAV", 1.1);	// Nível de Atendimento de Vazão [0.95..1.5]
	    functionBlock.setVariable("UH", 0.5);	// Grau de Utilização de Hastes [0..1]
	    functionBlock.setVariable("CH", 1.2);	// Custo da Coluna de Hastes [0.3..2.0]
	    functionBlock.setVariable("PT", 0.5);	// Peak de Torque [0..1]
	    functionBlock.setVariable("FB", 15);	// Frequência de Bombeio [0..25]
	    functionBlock.setVariable("CMax", 0.4);	// Carga Máxima [0..1]
	    ...
	    System.out.println("Sistema de Bombeio:" + functionBlock.getVariable("CDH").getValue());
	    //OR
